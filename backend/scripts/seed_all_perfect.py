...
async def seed_perfect():
    settings = get_settings()
    engine = create_async_engine(settings.database_url, echo=False)

    print("🚀 Seeding Complete NOVA Platform Data with explicit transaction management...")
    ...
    async with AsyncSession(engine) as session:
        ...
        res_s = await session.execute(select(User).where(User.email == "security@nova.ai"))
        sec_id = res_s.scalars().first().id
        res_d = await session.execute(select(User).where(User.email == "dev@nova.ai"))
        dev_id = res_d.scalars().first().id
    ...
    async with AsyncSession(engine) as session:
        async with session.begin():
            ...
            for name, url, prov, branch, sched in repo_specs:
                ...
                r = res.scalars().first()
                if not r:
                    r = Repository(name=name, url=url, provider=prov, default_branch=branch, scheduled_scan_enabled=sched, owner_id=admin_id)
                    session.add(r)
                else:
                    r.owner_id = admin_id
                    r.url = url
    ...
    async with AsyncSession(engine) as session:
        async with session.begin():
            ...
            for kw, resp, active, draft in faqs:
                session.add(FAQRule(keyword=kw, response=resp, is_active=active, is_draft=draft, created_by_id=admin_id))
    ...
    async with AsyncSession(engine) as session:
        async with session.begin():
            ...
            m3 = ChatMessage(session_id=s2.id, role="user", content="What cryptographic algorithms are mandated by PCI-DSS v4.0 Requirement 3.4 for storing cardholder data?", citations=[], confidence_vector={}, reasoning_trace={}, consensus_matrix={})
            m4 = ChatMessage(session_id=s2.id, role="assistant", content="PCI-DSS Requirement 3.4 mandates AES-256-GCM or RSA-4096 for all cardholder data storage.", citations=[], confidence_vector={"retrieval_confidence": 0.98}, calibrated_trust_score=0.984, reasoning_trace={"policy_gate": "GENERATE"}, consensus_matrix={"supports_count": 1, "c_agreement": 1.0})
    ...
    async with AsyncSession(engine) as session:
        async with session.begin():
            ...
            for act, rtype, rid, stat, det in audits:
                session.add(AuditLog(user_id=admin_id, user_email="admin@nova.ai", action=act, resource_type=rtype, resource_id=rid, status=stat, ip_address="127.0.0.1", user_agent="Mozilla/5.0", details=det, created_at=now - datetime.timedelta(minutes=30)))
            ...
            for feat, q, rc, ts, fb, lat in s_logs:
                session.add(SearchAnalyticsLog(feature=feat, query=q, result_count=rc, top_score=ts, fallback_triggered=fb, latency_ms=lat, user_id=admin_id))
