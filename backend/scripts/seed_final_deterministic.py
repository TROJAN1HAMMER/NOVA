...
async def seed_deterministic():
    settings = get_settings()
    engine = create_async_engine(settings.database_url, echo=False)

    print("🚀 Starting Deterministic Seed with explicit UUID generation...")
        ...
        async with session.begin():
            ...
            for email, name, role, pwd in user_specs:
                ...
                if not u:
                    ...
                    u.full_name = name
                    u.is_active = True
                    user_id_map[email] = u.id

    admin_id = user_id_map["admin@nova.ai"]
    sec_id = user_id_map["security@nova.ai"]
    dev_id = user_id_map["dev@nova.ai"]
    print(f"✅ Users ready. Admin ID: {admin_id}")
            ...
            for name, url, prov, branch, sched in repo_specs:
                ...
                if not r:
                    session.add(Repository(
                        id=uuid.uuid4(), name=name, url=url, provider=prov,
                        default_branch=branch, scheduled_scan_enabled=sched, owner_id=admin_id
                    ))
                else:
                    r.owner_id = admin_id
                    r.url = url
    ...
    docs = [
        ...
        ("Zero_Trust_Network_Architecture_Policy.md", "md", "1.0", zt_grp, True, "Architecture Team", ["zero-trust", "network", "cloud"], [
            ("ZT-01: Explicit Verification Principle", "Zero Trust > ZT-01", "Never trust, always verify. Every transaction and inter-service call must explicitly evaluate caller identity and scope."),
        ]),
    ]
        ...
        async with session.begin():
            for fname, dtype, ver, grp, is_lat, auth, tags, chunks_list in docs:
                ...
                if not doc:
                    ...
                    doc.uploaded_by_id = admin_id
                    doc.status = "indexed"
                    doc.version = ver
                    doc_id_map[fname] = doc.id

                # Replace chunks
                await session.execute(delete(KnowledgeChunk).where(KnowledgeChunk.document_id == doc_id_map[fname]))
                for idx, (head, spath, content) in enumerate(chunks_list):
                    ...
                    chunk = KnowledgeChunk(
                        id=uuid.uuid4(),
                        document_id=doc_id_map[fname], chunk_index=idx, content=content,
                        heading=head, section_path=spath, page_number=idx + 1,
                        token_count=len(content.split()), embedding=[x/norm for x in vec]
                    )
                    session.add(chunk)
    print("✅ Knowledge Documents & Chunks ready.")

    # 4. Knowledge Graph
    pci_doc_id = doc_id_map["PCI_DSS_v4.0_Banking_Security_Standard.pdf"]
    owasp_doc_id = doc_id_map["OWASP_Financial_API_Security_Guide.md"]
    nist_doc_id = doc_id_map["NIST_SP_800_53_Control_Baselines.pdf"]

    async with AsyncSession(engine) as session:
        async with session.begin():
            await session.execute(delete(KnowledgeRelation))
            await session.execute(delete(KnowledgeEntity))

            entities_data = [
                (pci_doc_id, "PCI-DSS v4.0 Standard", "STANDARD", 8),
                (pci_doc_id, "Requirement 3.4 (Cardholder Cryptography)", "RULE", 6),
                (pci_doc_id, "Requirement 6.4.1 (AST Injection Prevention)", "RULE", 5),
                (pci_doc_id, "Requirement 7.1 (RBAC & MFA)", "RULE", 7),
                (pci_doc_id, "Requirement 8.3 (MFA Enforcement)", "RULE", 4),
                (pci_doc_id, "AES-256-GCM / RSA-4096", "ALGORITHM", 6),
                (pci_doc_id, "Multi-Factor Authentication (MFA)", "CONTROL", 9),
                (owasp_doc_id, "OWASP FAPI Framework", "FRAMEWORK", 10),
                (owasp_doc_id, "FAPI-01 (BOLA)", "VULNERABILITY", 7),
                (owasp_doc_id, "FAPI-02 (Unprotected Transactions)", "VULNERABILITY", 8),
                (owasp_doc_id, "FAPI-03 (Token Exposure)", "VULNERABILITY", 5),
                (owasp_doc_id, "Resource Ownership Check", "CONTROL", 6),
                (owasp_doc_id, "RequireRole('admin')", "CONTROL", 9),
                (nist_doc_id, "NIST SP 800-53 Standard", "STANDARD", 6),
                (nist_doc_id, "AC-3 Access Enforcement", "RULE", 5),
                (nist_doc_id, "SC-13 Cryptographic Protection", "RULE", 5),
            ]
            ent_lookup = {}
            for did, ename, etype, mcount in entities_data:
                e_id = uuid.uuid4()
                ent = KnowledgeEntity(id=e_id, document_id=did, entity_name=ename, entity_type=etype, mention_count=mcount)
                session.add(ent)
                ent_lookup[ename] = e_id
            ...
            for src, tgt, rtype in relations_data:
                s_id, t_id = ent_lookup.get(src), ent_lookup.get(tgt)
                if s_id and t_id:
                    session.add(KnowledgeRelation(id=uuid.uuid4(), source_entity_id=s_id, target_entity_id=t_id, relation_type=rtype))
    print("✅ Knowledge Graph ready.")

    # 5. FAQ Rules & Gap Inbox
    async with AsyncSession(engine) as session:
        async with session.begin():
            await session.execute(delete(FAQRule))
            faqs = [
            ...
            for kw, resp, active, draft in faqs:
                session.add(FAQRule(id=uuid.uuid4(), keyword=kw, response=resp, is_active=active, is_draft=draft, created_by_id=admin_id))
    print("✅ FAQ Rules & Gap Inbox ready.")

    # 6. Assistant Sessions & Messages
    async with AsyncSession(engine) as session:
        async with session.begin():
            ...
            m2 = ChatMessage(
                id=uuid.uuid4(),
                session_id=s1_id, role="assistant",
                content=(
                ...
                citations=[
                    {"source_id": "sec-intel-001", "title": "Verified Risk Assessment: PRIVILEGE_ESCALATION_RISK", "file_path": "data/demo_repo/admin_transactions.py:42", "cwe_id": "CWE-285", "reliability_weight": 0.95},
                    {"source_id": "kb-fapi-02", "title": "OWASP Financial API Security Guide", "file_path": "data/knowledge_base/OWASP_Financial_API_Security_Guide.md", "reliability_weight": 0.98}
                ],
                confidence_vector={"retrieval_confidence": 0.94, "consensus_agreement": 0.96, "citation_coverage": 1.0, "reasoning_depth": 0.92, "freshness_factor": 1.0, "hallucination_risk": 0.02, "source_reliability": 0.96, "user_feedback_prior": 0.90},
                calibrated_trust_score=0.952,
                reasoning_trace={"pipeline_stage": "STAGE_2_SAFETY_GATE_PASS", "nli_consensus": "SUPPORTS", "policy_gate": "GENERATE"},
                consensus_matrix={"matrix_size": "2x2", "supports_count": 1, "contradicts_count": 0, "c_agreement": 0.96}
            )
            ...
            m3 = ChatMessage(id=uuid.uuid4(), session_id=s2_id, role="user", content="What cryptographic algorithms are mandated by PCI-DSS v4.0 Requirement 3.4 for storing cardholder data?", citations=[], confidence_vector={}, reasoning_trace={}, consensus_matrix={})
            m4 = ChatMessage(id=uuid.uuid4(), session_id=s2_id, role="assistant", content="PCI-DSS Requirement 3.4 mandates AES-256-GCM or RSA-4096 for all cardholder data storage.", citations=[], confidence_vector={"retrieval_confidence": 0.98}, calibrated_trust_score=0.984, reasoning_trace={"policy_gate": "GENERATE"}, consensus_matrix={"supports_count": 1, "c_agreement": 1.0})
            session.add_all([m3, m4])
    print("✅ Assistant Chat Sessions & Messages ready.")

    # 7. Audit Logs, Telemetry & Feedback
    async with AsyncSession(engine) as session:
        async with session.begin():
            await session.execute(delete(AuditLog))
            await session.execute(delete(SearchAnalyticsLog))
            await session.execute(delete(Feedback))

            now = datetime.datetime.now(datetime.timezone.utc)
            audits = [
                ("auth.login", "user", str(admin_id), "success", {"auth_method": "password"}),
            ...
            for act, rtype, rid, stat, det in audits:
                session.add(AuditLog(id=uuid.uuid4(), user_id=admin_id, user_email="admin@nova.ai", action=act, resource_type=rtype, resource_id=rid, status=stat, ip_address="127.0.0.1", user_agent="Mozilla/5.0", details=det, created_at=now - datetime.timedelta(minutes=30)))
            ...
            for feat, q, rc, ts, fb, lat in s_logs:
                session.add(SearchAnalyticsLog(id=uuid.uuid4(), feature=feat, query=q, result_count=rc, top_score=ts, fallback_triggered=fb, latency_ms=lat, user_id=admin_id))

            fbs = [
                ("assistant", "ref-chat-msg-01", 5, "Accurately pinpointed the missing RequireRole decorator in code.", admin_id),
                ("assistant", "ref-chat-msg-02", 5, "Clear explanation of PCI-DSS encryption requirements.", sec_id),
            ]
            for feat, ref, rat, comm, uid in fbs:
                session.add(Feedback(id=uuid.uuid4(), feature=feat, reference_id=ref, rating=rat, comment=comm, user_id=uid))
    print("✅ Audit Logs, Telemetry & Feedback ready.")

    # 8. Security Intelligence Context Graph, Assets & Assessments
    async with AsyncSession(engine) as session:
        async with session.begin():
            await session.execute(delete(SecurityIntelAssessment))
            await session.execute(delete(SecurityIntelRiskScenario))
            await session.execute(delete(SecurityIntelControl))
            await session.execute(delete(SecurityIntelObservation))
            await session.execute(delete(SecurityIntelAsset))
            ...
            a1 = SecurityIntelAsset(id=a1_id, asset_name="POST /admin/transactions", asset_type="ENDPOINT", criticality="HIGH", owner="Payments Engineering Team", location="data/demo_repo/admin_transactions.py:42", attributes={"protocol": "HTTPS", "framework": "FastAPI", "category": "FINANCIAL_API"})
            a2 = SecurityIntelAsset(id=a2_id, asset_name="TransactionController", asset_type="SERVICE", criticality="CRITICAL", owner="Core Banking Architecture", location="data/demo_repo/transaction_controller.py:12", attributes={"language": "Python 3.10", "type": "Microservice"})
            a3 = SecurityIntelAsset(id=a3_id, asset_name="AuthService", asset_type="APPLICATION", criticality="CRITICAL", owner="IAM Security Team", location="data/demo_repo/auth_service.py:1", attributes={"auth_standard": "OAuth2 / JWT", "mfa": True})
            a4 = SecurityIntelAsset(id=a4_id, asset_name="DATABASE: banking_core_db", asset_type="DATABASE", criticality="CRITICAL", owner="DBA Infrastructure Team", location="postgresql://core_db:5432/banking", attributes={"engine": "PostgreSQL 15", "encryption_at_rest": "AES-256"})
            a5 = SecurityIntelAsset(id=a5_id, asset_name="POST /transfers/wire", asset_type="ENDPOINT", criticality="CRITICAL", owner="Wire Operations", location="data/demo_repo/wire_transfer.py:88", attributes={"limit_check": True, "dual_control": True})
            session.add_all([a1, a2, a3, a4, a5])

            obs1 = SecurityIntelObservation(id=uuid.uuid4(), asset_id=a1_id, observation_type="PUBLIC_ENDPOINT", location="data/demo_repo/admin_transactions.py:42", evidence_span="@router.post('/admin/transactions')", confidence=0.98, provenance="ast_decorator_visitor", attributes={"route": "/admin/transactions", "method": "POST"})
            obs2 = SecurityIntelObservation(id=uuid.uuid4(), asset_id=a1_id, observation_type="AUTHORIZATION_BOUNDARY", location="data/demo_repo/admin_transactions.py:44", evidence_span="def process_admin_transaction(tx_payload: TransactionSchema):", confidence=0.95, provenance="ast_arg_visitor", attributes={"missing_role_decorator": True})
            obs3 = SecurityIntelObservation(id=uuid.uuid4(), asset_id=a1_id, observation_type="DATABASE_ACCESS", location="data/demo_repo/admin_transactions.py:65", evidence_span="await db.execute(insert(TransactionModel)...)", confidence=0.92, provenance="ast_call_visitor", attributes={"target_table": "transactions"})

            c1 = SecurityIntelControl(id=uuid.uuid4(), asset_id=a1_id, control_type="AUTHORIZATION", scope="POST /admin/transactions", state="PRESENT", evidence="dependencies=[Depends(RequireRole('admin'))]", confidence=0.96)
            c2 = SecurityIntelControl(id=uuid.uuid4(), asset_id=a1_id, control_type="AUTHENTICATION", scope="POST /admin/transactions", state="PRESENT", evidence="Depends(get_current_active_user)", confidence=0.98)
            c3 = SecurityIntelControl(id=uuid.uuid4(), asset_id=a1_id, control_type="INPUT_VALIDATION", scope="POST /admin/transactions", state="PRESENT", evidence="tx_payload: TransactionSchema", confidence=0.95)

            sc1 = SecurityIntelRiskScenario(id=uuid.uuid4(), asset_id=a1_id, scenario_type="PRIVILEGE_ESCALATION_RISK", trust_boundary_crossed="INTERNET -> APPLICATION_CORE", attack_path={"steps": ["1. User authenticates", "2. Dispatches POST /admin/transactions", "3. Transaction processed without admin role check"]}, exposure_signal="PUBLIC_ENDPOINT_WITH_ELEVATED_PRIVILEGE", control_status="AUTHORIZATION_PRESENT_VERIFIED", potential_impact="Unauthorized balance adjustment or privilege escalation across banking accounts.", verification_state="VERIFIED")

            ass1 = SecurityIntelAssessment(id=uuid.uuid4(), asset_id=a1_id, risk_type="PRIVILEGE_ESCALATION_RISK", severity="HIGH", confidence=0.95, affected_scope="data/demo_repo/admin_transactions.py:42-70", evidence_chain=[{"fact": "POST endpoint discovered in AST", "line": 42}, {"fact": "RequireRole('admin') attached and verified", "line": 44}], attack_path=["Client sends unprivileged HTTP request", "Gateway evaluates role dependency", "Access granted to verified admin"], controls_evaluated=[{"type": "AUTHORIZATION", "state": "PRESENT", "confidence": 0.96}], reasoning="The administrative endpoint POST /admin/transactions was verified fixed with RequireRole('admin').", remediation="Apply @router.post('/admin/transactions', dependencies=[Depends(RequireRole('admin'))])", status="VERIFIED_FIXED", commit_hash="f68521e")

            session.add_all([obs1, obs2, obs3, c1, c2, c3, sc1, ass1])
    print("✅ Security Intelligence Context Graph ready.")

    # 9. Posture Snapshots
    async with AsyncSession(engine) as session:
        async with session.begin():
            await session.execute(delete(SecurityIntelPostureSnapshot))
            now = datetime.datetime.now(datetime.timezone.utc)
            posture_records = [
    ...
    print("\n🎉 ALL PLATFORM DATA SEEDED DETERMINISTICALLY AND VERIFIED!")

if __name__ == "__main__":
    asyncio.run(seed_deterministic())
