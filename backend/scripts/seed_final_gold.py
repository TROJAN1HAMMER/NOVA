...
async def seed():
    print("🌟 SEEDING FULL PLATFORM DATA ACROSS ALL 12 SCREENS...")
    ...
    async with AsyncSessionLocal() as session:
        for email, name, role, pwd in user_specs:
            ...
            if not u:
                u_id = uuid.uuid4()
                u = User(
                    id=u_id, email=email, full_name=name,
    ...
    admin_id = user_map["admin@nova.ai"]
    sec_id = user_map["security@nova.ai"]
    dev_id = user_map["dev@nova.ai"]
    print(f"✅ Users ready. Admin: {admin_id}")
        ...
        for fname, dtype, ver, grp, is_lat, auth, tags, chunks_list in docs:
            ...
            if not doc:
                ...
                doc = KnowledgeDocument(
                    id=d_id, filename=fname, document_type=dtype, version=ver,
    ...
    print("\n🎉 ALL PLATFORM DATA 100% PERSISTED SUCCESSFULLY!")
