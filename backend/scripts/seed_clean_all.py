...
async def seed_clean_all():
    ...
    async with AsyncSessionLocal() as session:
        ...
        users = [
            ...
            ("security@nova.ai", "Sarah Chen (Security Lead)", UserRole.SECURITY_ENGINEER, "SecurityPass123!"),
            ("dev@nova.ai", "Alex Rivera (Senior Dev)", UserRole.DEVELOPER, "DevPassword123!"),
            ("auditor@nova.ai", "Elena Rostova (Lead Auditor)", UserRole.AUDITOR, "AuditorPass123!"),
        ...
        admin = user_map["admin@nova.ai"]
        sec = user_map["security@nova.ai"]
        dev = user_map["dev@nova.ai"]
        print(f"✅ Users ready. Admin: {admin.email} (ID: {admin.id})")

        print("🌱 Seeding Repositories for Source Studio...")
        repos = [
        ...
        for name, url, provider, branch, sched in repos:
        ...
        print("🌱 Seeding Knowledge Documents & Chunks...")
        pci_grp = uuid.uuid4()
        fapi_grp = uuid.uuid4()
        nist_grp = uuid.uuid4()
        docs = [
        ...
        print("🌱 Seeding Knowledge Graph (Entities & Relations)...")
        pci_d = doc_map["PCI_DSS_v4.0_Banking_Security_Standard.pdf"]
        owasp_d = doc_map["OWASP_Financial_API_Security_Guide.md"]
        nist_d = doc_map["NIST_SP_800_53_Control_Baselines.pdf"]
        ...
        for did, ename, etype, mcount in entities_data:
            res = await session.execute(select(KnowledgeEntity).where(KnowledgeEntity.entity_name == ename))
        ...
        for src, tgt, rtype in relations_data:
            s_ent, t_ent = ent_map.get(src), ent_map.get(tgt)
            if s_ent and t_ent:
                res = await session.execute(select(KnowledgeRelation).where(
                ...
                if not res.scalars().first():
                    rel = KnowledgeRelation(source_entity_id=s_ent.id, target_entity_id=t_ent.id, relation_type=rtype)
                    session.add(rel)
        await session.commit()
        print("✅ Knowledge Graph ready.")

        print("🌱 Seeding Stage 0 FAQ Rules & Knowledge Gap Inbox...")
        faqs = [
        ...
        for kw, resp, active, draft in faqs:
            res = await session.execute(select(FAQRule).where(FAQRule.keyword == kw))
            f = res.scalars().first()
            if not f:
                f = FAQRule(keyword=kw, response=resp, is_active=active, is_draft=draft, created_by_id=admin.id)
                session.add(f)
            else:
                f.is_active = active
                f.is_draft = draft
        ...
        print("🌱 Seeding Audit Logs, Search Analytics & Feedback...")
        ...
        for act, rtype, rid, stat, det in audits:
            session.add(AuditLog(
                user_id=admin.id, user_email=admin.email, action=act,
                resource_type=rtype, resource_id=rid, status=stat,
                ip_address="127.0.0.1", user_agent="Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7)",
                details=det, created_at=now - datetime.timedelta(minutes=30)
            ))
        ...
        for feat, q, rc, ts, fb, lat in s_logs:
            session.add(SearchAnalyticsLog(
                feature=feat, query=q, result_count=rc, top_score=ts,
                fallback_triggered=fb, latency_ms=lat, user_id=admin.id
            ))

        fbs = [
            ("assistant", "ref-chat-msg-01", 5, "Accurately pinpointed the missing RequireRole decorator in code.", admin.id),
            ("assistant", "ref-chat-msg-02", 5, "Clear explanation of PCI-DSS encryption requirements.", sec.id),
        ]
        ...
        print("🌱 Seeding Security Intelligence Context Graph, Assets & Assessments...")
        res_a = await session.execute(select(SecurityIntelAsset))
        existing_assets = res_a.scalars().all()
        if len(existing_assets) < 5:
            a1 = SecurityIntelAsset(
                asset_name="POST /admin/transactions", asset_type="ENDPOINT",
                criticality="HIGH", owner="Payments Engineering Team",
            ...
            a2 = SecurityIntelAsset(
                asset_name="TransactionController", asset_type="SERVICE",
                criticality="CRITICAL", owner="Core Banking Architecture",
            ...
            a3 = SecurityIntelAsset(
                asset_name="AuthService", asset_type="APPLICATION",
                criticality="CRITICAL", owner="IAM Security Team",
                location="data/demo_repo/auth_service.py:1",
                attributes={"auth_standard": "OAuth2 / JWT", "mfa": True}
            )
            a4 = SecurityIntelAsset(
                asset_name="DATABASE: banking_core_db", asset_type="DATABASE",
                criticality="CRITICAL", owner="DBA Infrastructure Team",
                location="postgresql://core_db:5432/banking",
                attributes={"engine": "PostgreSQL 15", "encryption_at_rest": "AES-256"}
            )
            a5 = SecurityIntelAsset(
                asset_name="POST /transfers/wire", asset_type="ENDPOINT",
                criticality="CRITICAL", owner="Wire Operations",
            ...
            obs1 = SecurityIntelObservation(
                asset_id=a1.id, observation_type="PUBLIC_ENDPOINT",
                location="data/demo_repo/admin_transactions.py:42",
                evidence_span="@router.post('/admin/transactions')",
                confidence=0.98, provenance="ast_decorator_visitor",
                attributes={"route": "/admin/transactions", "method": "POST"}
            )
            obs2 = SecurityIntelObservation(
                asset_id=a1.id, observation_type="AUTHORIZATION_BOUNDARY",
                location="data/demo_repo/admin_transactions.py:44",
                evidence_span="def process_admin_transaction(tx_payload: TransactionSchema):",
                confidence=0.95, provenance="ast_arg_visitor",
                attributes={"missing_role_decorator": True}
            )
            obs3 = SecurityIntelObservation(
                asset_id=a1.id, observation_type="DATABASE_ACCESS",
                location="data/demo_repo/admin_transactions.py:65",
                evidence_span="await db.execute(insert(TransactionModel)...)",
                confidence=0.92, provenance="ast_call_visitor",
                attributes={"target_table": "transactions"}
            )

            c1 = SecurityIntelControl(
                asset_id=a1.id, control_type="AUTHORIZATION", scope="POST /admin/transactions",
                state="PRESENT", evidence="dependencies=[Depends(RequireRole('admin'))]", confidence=0.96
            )
            c2 = SecurityIntelControl(
                asset_id=a1.id, control_type="AUTHENTICATION", scope="POST /admin/transactions",
                state="PRESENT", evidence="Depends(get_current_active_user)", confidence=0.98
            )
            c3 = SecurityIntelControl(
                asset_id=a1.id, control_type="INPUT_VALIDATION", scope="POST /admin/transactions",
                state="PRESENT", evidence="tx_payload: TransactionSchema", confidence=0.95
            )

            sc1 = SecurityIntelRiskScenario(
                asset_id=a1.id, scenario_type="PRIVILEGE_ESCALATION_RISK",
                trust_boundary_crossed="INTERNET -> APPLICATION_CORE",
                attack_path={"steps": ["1. User authenticates", "2. Dispatches POST /admin/transactions", "3. Transaction processed without admin role check"]},
            ...
            ass1 = SecurityIntelAssessment(
                asset_id=a1.id, risk_type="PRIVILEGE_ESCALATION_RISK", severity="HIGH",
                confidence=0.95, affected_scope="data/demo_repo/admin_transactions.py:42-70",
                evidence_chain=[{"fact": "POST endpoint discovered in AST", "line": 42}, {"fact": "RequireRole('admin') attached and verified", "line": 44}],
                attack_path=["Client sends unprivileged HTTP request", "Gateway evaluates role dependency", "Access granted to verified admin"],
                controls_evaluated=[{"type": "AUTHORIZATION", "state": "PRESENT", "confidence": 0.96}],
                reasoning="The administrative endpoint POST /admin/transactions was verified fixed with RequireRole('admin').",
                remediation="Apply @router.post('/admin/transactions', dependencies=[Depends(RequireRole('admin'))])",
                status="VERIFIED_FIXED", commit_hash="f68521e"
            )
        ...
        print("🌱 Seeding Posture Trajectory Snapshots for Executive Radar...")
        res_posture = await session.execute(select(SecurityIntelPostureSnapshot))
        if not res_posture.scalars().first():
            posture_records = [
    ...
    print("\n🎉 ALL SCREENS FULLY SEEDED AND VERIFIED!")

if __name__ == "__main__":
    asyncio.run(seed_clean_all())
